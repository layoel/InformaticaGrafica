...
//para saber si he calculado ya las normales y no volver a calcularlas:
  b_normales_caras = false;
  b_normales_vertices = false;

  //ya no necesito darle valores en el constructor porque los asigno al dibjar el objeto
   ambiente_difusa[0] = 0.7;
   ambiente_difusa[1] = 0.7;
   ambiente_difusa[2] = 0.7;
...
void _triangulos3D::draw_solido_ajedrez(float r1, float g1, float b1, float r2, float g2, float b2)
{
  ...
  glColor3f(r1,g1,b1);
  glBegin(GL_TRIANGLES);
  for (size_t  i=0;i<caras.size();i=i+2){
    glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
    glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
    glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
    }
  ...
  glColor3f(r2,g2,b2);
  glBegin(GL_TRIANGLES);
  for (size_t  i=1;i<caras.size();i=i+2){
    glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
    glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
    glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
    }
...
//calculo las normales
  for (size_t i = 0; i < caras.size(); i++) {
        ...
        _vertex3f v2=vertices[caras[i]._2]-vertices[caras[i]._0];
...
void _rotacion::parametros(vector<_vertex3f> perfil, int num, char eje) {
    ...
    for (j=0; j<num; j++){
      ...
      for (i=0; i<num_aux-1; i++){
        ...
        cara_aux._2 = i + 1 + ((j + 1) % num) * num_aux;
        cara_aux._1 = i + 1 + j *num_aux;
    ...
    if (eje == 'y'){
      ...
      if (fabs(perfil[num_aux-1].x)>0.0){
        ...
        for (j=0; j<num; j++){
          ...
          cara_aux._2 = (vertices.size() - 1);
          cara_aux._1 = (num_aux - 1) + ((j + 1) % num) * num_aux;
          caras.push_back(cara_aux);
        }
